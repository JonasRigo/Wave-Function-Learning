...
class AFH_Positive(object):
    def __init__(self,N = 8, J = 1., g = 0., Sztot = 0):
        self.N = N
        self.J = J
        self.g = g
        self.Sztot = 0
    ...
    def getH(self):
        states = np.empty((0, self.N), dtype=np.uint8)
        for state in np.array(list(itertools.product([-1, 1], repeat=self.N))):
            if np.sum(state) == self.Sztot:
